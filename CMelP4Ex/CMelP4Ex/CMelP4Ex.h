#pragma once
#ifdef CMELP4EX_EXPORTS  
#define CMELP4EX __declspec(dllexport)   
#else  
#define CMELP4EX __declspec(dllimport)   
#endif  
...
extern "C"
{
	CMELP4EX BOOL Init(int portNo, int baudrate, int databit, int stopbit, int parity, int wait, BOOL ctrl_line = TRUE)
	{
		CMelsecP4Ex *melsec = new CMelsecP4Ex();
		return melsec->Init(portNo, baudrate, databit, stopbit, parity, wait, ctrl_line);
	}
	CMELP4EX void ComClose(void)
	{
		CMelsecP4Ex *melsec = new CMelsecP4Ex();
		return melsec->ComClose();
	}

	CMELP4EX int P4WriteW(char* station, char* pcstation, char* addr, short counts, char* msg)
	{
		CMelsecP4Ex *melsec = new CMelsecP4Ex();
		return melsec->P4WriteW(station, pcstation, addr, counts, msg);
	}
	CMELP4EX int P4ReadW(char* buf, int bufsize, char* station, char* pcstation, char* addr, short counts)
	{
		CMelsecP4Ex *melsec = new CMelsecP4Ex();
		return melsec->P4ReadW(buf, bufsize, station, pcstation, addr, counts);
	}
	CMELP4EX int P2C4WriteW(char* station, char* pcstation, char* addr, short counts, char* msg)
	{
		CMelsecP4Ex *melsec = new CMelsecP4Ex();
		return melsec->P2C4WriteW(station, pcstation, addr, counts, msg);
	}
	CMELP4EX int P2C4ReadW(char* buf, int bufsize, char* station, char* pcstation, char* addr, short counts)
	{
		CMelsecP4Ex *melsec = new CMelsecP4Ex();
		return melsec->P2C4ReadW(buf, bufsize, station, pcstation, addr, counts);
	}

	CMELP4EX int P4Send(char* station, char* pcstation, char* command, char* addr, short counts, char* msg)
	{
		CMelsecP4Ex *melsec = new CMelsecP4Ex();
		return melsec->P4Send(station, pcstation, command, addr, counts, msg);
	}
	CMELP4EX int P4WriteB(char* station, char* pcstation, char* addr, int onoff)
	{
		CMelsecP4Ex *melsec = new CMelsecP4Ex();
		return melsec->P4WriteB(station, pcstation, addr, onoff);
	}
	CMELP4EX int P4ReadB(char* station, char* pcstation, char* addr)
	{
		CMelsecP4Ex *melsec = new CMelsecP4Ex();
		return melsec->P4ReadB(station, pcstation, addr);
	}
	CMELP4EX int P4ReadBB(char* buf, int bufsize, char* station, char* pcstation, char* addr, short counts)
	{
		CMelsecP4Ex *melsec = new CMelsecP4Ex();
		return melsec->P4ReadBB(buf, bufsize, station, pcstation, addr, counts);
	}
	CMELP4EX int P4WriteBB(char* station, char* pcstation, char* addr, int cnt, char* dat)
	{
		CMelsecP4Ex *melsec = new CMelsecP4Ex();
		return melsec->P4WriteBB(station, pcstation, addr, cnt, dat);
	}
	CMELP4EX int Str2Mel(char* buf, unsigned int bufsize, char* str)
	{
		CMelsecP4Ex *melsec = new CMelsecP4Ex();
		return melsec->Str2Mel(buf, bufsize, str);
	}
	CMELP4EX int Mel2Str(char* buf, unsigned int bufsize, char* melstr)
	{
		CMelsecP4Ex *melsec = new CMelsecP4Ex();
		return melsec->Mel2Str(buf, bufsize, melstr);
	}
	CMELP4EX int UStr2Mel(char* buf, unsigned int bufsize, unsigned char* str)
	{
		CMelsecP4Ex *melsec = new CMelsecP4Ex();
		return melsec->UStr2Mel(buf, bufsize, str);
	}
	CMELP4EX BOOL ULChg(char* buf)
	{
		CMelsecP4Ex *melsec = new CMelsecP4Ex();
