...
void TOdbcTable::Reset(void)
{
	...
	if( m_hStmt != NULL ){
		...
		if( IsSuccess(ret) ){
			SQLSetLastError(SQL_HANDLE_STMT,m_hStmt, "Reset00");
		}
		
		ret = SQLFreeHandle( SQL_HANDLE_STMT, m_hStmt );
		if( IsSuccess(ret) ){
			SQLSetLastError(SQL_HANDLE_STMT,m_hStmt, "Reset01");
		}
	}
...
{
	char buf[100],cmpstr[50];
	SQLSMALLINT nSize;

	if( m_hConn == NULL ){
		m_fetchScroll = TRUE;
		m_hConn = *hconn;
		//iSeriesのODBCドライバのバージョンを取ってきて、Sel時のFetch関数を決定する
		//SQL_DRIVER_NAME,SQL_DRIVER_VER,SQL_DRIVER_ODBC_VER
		strcpy(cmpstr,"05.04.0000");
		if( SQL_SUCCESS == SQLGetInfo(m_hConn,SQL_DRIVER_VER,buf,sizeof(buf)-1,&nSize) ){
			if( !memcmp( buf, cmpstr,strlen(cmpstr) ) ) m_fetchScroll = FALSE;
		}
	}
	return(0);
}
//---------------------------------------------------------------------------------------------------------------------
//	void TOdbcTable::EnableFethcScroll(BOOL on)
//	type:Public
//	Outline:Enable or disable fetch scroll function.
//	Arg 1 :on or off
//  *note Before call this function, call set DB function.
//---------------------------------------------------------------------------------------------------------------------
void TOdbcTable::EnableFethcScroll(BOOL on)
{
	m_fetchScroll=on;
...
int TOdbcTable::MoveNext(void)
{
	...
	//2012.02.15 matsuda この関数内でBindされているフィールド（構造体）のクリアを行う
	ClearBindedData();
...
int TOdbcTable::Sel(char* sql)
{
	...
	//Attach的な使い方
	// 2009.10.16 matuda SQLFetchをClientAccessODBCﾄﾞﾗｲﾊﾞ11.0.0.8で使用すると飛ぶので使用禁止(ODBC1.0の関数だからかも）
	// ↑だったのだけど、ドライバのバージョンでどちらを使うかを切り替えるよう変更 2009.11.05 ﾏﾂﾀﾞ
	if( m_fetchScroll == FALSE ){
//		MessageBox(0,"fetch",0,0);
		ret = SQLFetch(m_hStmt);
	}
	else{
//		MessageBox(0,"fetch scroll",0,0);
		ret = SQLFetchScroll( m_hStmt,SQL_FETCH_NEXT,0 );
	}
...
//	引数２：影響した行数
//---------------------------------------------------------------------------------------------------------------------
int TOdbcTable::Exec(char* sql,int *affect_rows)
{
	SQLRETURN ret;
	SQLHANDLE stmt;
	SQLLEN rowcount;
...
*/
	//2010.02.04 matuda 影響した行を返すよう変更
	ret = SQLRowCount( stmt,&rowcount );
	if( IsSuccess(ret) ){
		SQLSetLastError(SQL_HANDLE_STMT,stmt, "EXEC03");
		*affect_rows=-1;
	}
	*affect_rows= rowcount;
...
//---------------------------------------------------------------------------------------------------------------------
//	void TOdbcTable::ClearBindedData(void)
//	タイプ：public
//	機能　：Bindされているフィールドをクリアするためテーブル基底クラスで利用下さい
//	Virtualのダミー関数
//	2012.02.15 matsuda
//---------------------------------------------------------------------------------------------------------------------
void TOdbcTable::ClearBindedData(void)
