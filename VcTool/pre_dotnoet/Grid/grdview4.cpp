...
BOOL CGridViewCtrl::CreateGrid(LPRECT pRect,GRIDINFO* gr,char* fontname,DWORD charset)
{
	...
	m_DispRows = (pRect->bottom / m_ColHeight ) -1;
...
void CGridViewCtrl::ShowGrid(int nCmdShow )
{
	if( nCmdShow != SW_SHOW && nCmdShow != SW_HIDE ) return;

	ShowWindow(m_ListWnd,nCmdShow);
	if( nCmdShow==SW_HIDE )
...
BOOL CGridViewCtrl::EndEdit(int action)
{
	...
	if( action > 0 ){
		...
		switch( color ){
			case 0:rgb_color=RGB(0,0,0);break;			//Black
			case 1:rgb_color=RGB(255,0,0);break;		//Red
			case 2:rgb_color=RGB(0,0,255);break;		//Blue
			case 3:rgb_color=RGB(0,128,0);break;	//Green
			case 4:rgb_color=RGB(128,0,128);break;	//Purple
			default:rgb_color=GetCelColEx(m_editX,m_editY);break;
		}
	}
...
//		if( !SetCel( m_editX,m_editY,theData,TGridData::GetCelTag(m_editX,m_editY), TRUE ,color,-1)) return(FALSE);
		if( !SetCelEx( m_editX,m_editY,theData,TGridData::GetCelTag(m_editX,m_editY), TRUE ,rgb_color,GetCelBkColEx(m_editX,m_editY))) return(FALSE);
		// 移動先が有効な場合はそこに飛ぶ
...
//	m_CellBuf.bkcolor=TGridData::m_StrData[ m_ActCel.row].bkcolor[ m_ActCel.col];
	m_CellBuf.ncolor=TGridData::m_StrData[ m_ActCel.row].ncolor[ m_ActCel.col];
	m_CellBuf.nbkcolor=TGridData::m_StrData[ m_ActCel.row].nbkcolor[ m_ActCel.col];
	return(TRUE);
...
//	if( !TGridData::SetCelData(  m_ActCel.col, m_ActCel.row, m_CellBuf.color,m_CellBuf.tag,m_CellBuf.data,m_CellBuf.bkcolor)) return(FALSE);
	if( !TGridData::SetCelDataEx(  m_ActCel.col, m_ActCel.row, m_CellBuf.ncolor,m_CellBuf.tag,m_CellBuf.data,m_CellBuf.nbkcolor)) return(FALSE);
...
//---------------------------------------------------------------------------------------------------------------------
//	BOOL CGridViewCtrl::ChangeRowTitle2(long rowNo, char* newtitle)
//	タイプ：public
//	機能　：行題の変更
//	引数１：変更する行
//	引数２：変更後の行題
...
BOOL CGridViewCtrl::ChangeRowTitle2(long rowNo, char* newtitle)
{
	if( !TGridData::ChangeRowTitle2(rowNo, newtitle)) return(FALSE);
...
//---------------------------------------------------------------------------------------------------------------------
//	BOOL CGridViewCtrl::SetCel(int col,int row, char* data,BOOL draw);
//	タイプ：public
//	機能　：セルにデータを設定
//	引数１：設定する列番号
//	引数２：設定する行番号
//	引数３：設定データ
//	引数４：画面に反映＝TRUE,反映なし=FALSE
//	戻り値：成功＝TRUE
//---------------------------------------------------------------------------------------------------------------------
BOOL CGridViewCtrl::SetCel(int col,int row, char* data,BOOL draw)
{
	// データをセット
	if(!TGridData::SetCelData(col,row,data)) return(FALSE);
...
//	戻り値：成功＝TRUE
//---------------------------------------------------------------------------------------------------------------------
BOOL CGridViewCtrl::SetCelEx(int col,int row, char* data,long cTag,BOOL draw,COLORREF color,COLORREF bkcolor)
{
	// データをセット
	if(!TGridData::SetCelDataEx(col,row,color,cTag,data,bkcolor)) return(FALSE);
...
//	else FillRect( m_hdcList,&rect,(HBRUSH)GetStockObject(GRAY_BRUSH));
		...
		HBRUSH hbrush=CreateSolidBrush(m_StrData[row].nbkcolor[col]);
		FillRect( m_hdcList,&rect,hbrush);
		DeleteObject(hbrush);
	}
...
//			SetTextColor( m_hdcList, old );
//		}
		old = SetTextColor( m_hdcList, m_StrData[row].ncolor[col] );
		DrawText(m_hdcList,txtBuf,-1,&rect,uFlags);
		SetTextColor( m_hdcList, old );
	}
	SetRect(&rect	,xPos+1			// 行ヘッダの幅offset
...
//★
//	if(!TGridData::m_StrData[row].bkcolor[col])
//		FillRect( m_hdcList,&rect,(HBRUSH)GetStockObject(WHITE_BRUSH));
//	else
//		FillRect( m_hdcList,&rect,(HBRUSH)GetStockObject(GRAY_BRUSH));
	HBRUSH hbrush=CreateSolidBrush(m_StrData[row].nbkcolor[col]);
	FillRect( m_hdcList,&rect,hbrush);
	DeleteObject(hbrush);
...
//				else FillRect( m_hdcList,&rect,(HBRUSH)GetStockObject(GRAY_BRUSH));
					...
					//色を指定出来るように変更
					HBRUSH hbrush=CreateSolidBrush(TGridData::m_StrData[datrow].nbkcolor[col]);
					FillRect( m_hdcList,&rect,hbrush);
					DeleteObject(hbrush);
				}
				// 領域内に収まる文字数だけ出力
...
//						uFlags = DT_CENTER|DT_SINGLELINE|DT_WORDBREAK;
				...
				// 通常のセル
					...
					// 色つき
					old = SetTextColor( m_hdcList, TGridData::m_StrData[datrow].ncolor[col] );
					DrawText(m_hdcList,txtBuf,-1,&rect,uFlags);
					SetTextColor( m_hdcList, old );
				}
				//次のx座標
...
void CGridViewCtrl::EvVScroll(HWND hWnd,WORD event,WORD lastPos)
{
	...
	switch(event){
		case SB_LINEDOWN:
			...
			break;
		case SB_PAGEDOWN:
			nextTopRow+=m_DispRows;
//	if( TGridData::m_LastRowNo < m_DispRows ){
//			if( nextTopRow> )nextTopRow=;
			break;
		case SB_LINEUP:
			nextTopRow--;
			break;
...
BOOL CGridViewCtrl::MoveActiveCel(long newCol,long newRow,BOOL internal)
{
	...
	if( newRow > TGridData::m_LastRowNo || newCol >=TGridData::m_TotalCols ||
		newRow < m_FixRows || newCol < 0 )
		return(FALSE);

	// 移動先が画面に入ってい無い場合はスクロール
	if( newCol < m_Left || newCol > m_Right || 
...
{
	// 移動先のセルは有効か？
	if( newCol >=TGridData::m_TotalCols || newCol < 0 )
		return(FALSE);
...
//---------------------------------------------------------------------------------------------------------------------
//	void CGridViewCtrl::EvHScrollEnd(HWND hWnd,WORD event,WORD lastpos)
//	タイプ：virtual	(private)
//	機能　：アクティブセルが移動完了時に呼ばれる関数
//	引数１：移動後の列番号
//	引数２：移動後行番号
//	戻り値：デフォルト処理をさせる＝TRUEしない=FALSE
//---------------------------------------------------------------------------------------------------------------------
void CGridViewCtrl::EvHScrollEnd(HWND hWnd,WORD event,WORD lastpos,long left)
{
	SetFocus(m_ListWnd);
	return;
}
...
//		case WM_IME_KEYDOWN:
		...
		case WM_MOUSEWHEEL:
			if(m_DispRows>TGridData::m_LastRowNo ) break;

			if( HIWORD(wParam)>HIWORD(lParam))	EvVScroll(hWind,SB_LINEDOWN,HIWORD(wParam));
			else								EvVScroll(hWind,SB_LINEUP,HIWORD(wParam));
			break;
		...
		case WM_HSCROLL:
			...
			EvHScrollEnd(hWind,LOWORD(wParam),HIWORD(wParam),m_Left);
			break;
...
//	機能　：キーフック
//---------------------------------------------------------------------------------------------------------------------
BOOL CGridViewCtrl::PushActiveCell()
{
	GetActiveCel( &pushx , &pushy );
	return TRUE;
}
...
//	機能　：キーフック
//---------------------------------------------------------------------------------------------------------------------
BOOL CGridViewCtrl::PopActiveCell()
{
	MoveActiveCel( pushx , pushy );
	return TRUE;
}

//---------------------------------------------------------------------------------------------------------------------
//	LONG CGridViewCtrl::LONG CGridViewCtrl::GetCellWidth( long col );
//	タイプ：Public
//	機能　：Ｃｅｌｌの巾を返す
//---------------------------------------------------------------------------------------------------------------------
LONG CGridViewCtrl::GetCellWidth( long col )
{
	if( col < 0 || col >= m_TotalCols ) return -1;
	return m_ColInfo[col].width;
}

//---------------------------------------------------------------------------------------------------------------------
//	void CGridViewCtrl::LONG CGridViewCtrl::SetCellWidth( long col,long width );
//	タイプ：Public
//	機能　：Ｃｅｌｌの巾を返す
//---------------------------------------------------------------------------------------------------------------------
void CGridViewCtrl::SetCellWidth( long col,long width )
{
	if( col < 0 || col >= m_TotalCols ) return;
	m_ColInfo[col].width=width;
}

//---------------------------------------------------------------------------------------------------------------------
//	void CGridViewCtrl::LONG CGridViewCtrl::SetLeftCell( long col );
//	タイプ：Public
//	機能　：左端のCol位置を変更する
//---------------------------------------------------------------------------------------------------------------------
void CGridViewCtrl::SetLeftCell( long col )
{
	m_Left=col;
	DrawData();
...
//---------------------------------------------------------------------------------------------------------------------
//	void CGridViewCtrl::LONG CGridViewCtrl::GetLeftCell( long *col );
//	タイプ：Public
//	機能　：左端のCol位置を取得する
//---------------------------------------------------------------------------------------------------------------------
void CGridViewCtrl::GetLeftCell( long *col )
{
	*col=m_Left;
...
//---------------------------------------------------------------------------------------------------------------------
//	void CGridViewCtrl::SetTopCell( long row )
//	タイプ：Public
//	機能　：上端のCol位置を変更する
//---------------------------------------------------------------------------------------------------------------------
void CGridViewCtrl::SetTopCell( long row )
...
//---------------------------------------------------------------------------------------------------------------------
//	void CGridViewCtrl::GetTopCell( long *row )
//	タイプ：Public
//	機能　：上端のCol位置を取得する
//---------------------------------------------------------------------------------------------------------------------
void CGridViewCtrl::GetTopCell( long *row )
