...
//	引数２：印字速度 2～6
//---------------------------------------------------------------------------------------------------------------------
int TSatoPrinter::StartLabel(int mode,char speed, BOOL ignore_sensor /*=FALSE*/)
{
	...
	//センサ位置を無視するかどうかの設定を追加
	if(ignore_sensor == TRUE ){
...
//	引数　：ｺﾋﾟｰ枚数
//---------------------------------------------------------------------------------------------------------------------
int TSatoPrinter::EndLabel(long page, BOOL ignore_sensor /*=FALSE*/)
{
	if( m_hPrinter == NULL ) return(-1);
...
int	 TSatoPrinter::DrawKanjiPT( int x,int y,char* str, int len,short size ,short yoko,short tate,short pitch)
{
	...
	for( i=0; i< len; i++){
		...
		// 半角の場合はそのまま送る
			...
			//半角カナの場合に印字できない不具合を修正	//2014.09.15 sanno
			sprintf( cmd,"%02X",(unsigned char)str[i]);
