...
//	引数４：int pixPitch 最小ピクセル数(Exで違う箇所)
//	引数５：Ｘ座標
//	引数６：Ｙ座標
//		QRコードを印刷する
//	Caution!! 2バイト文字に対応していません!！ ﾏﾂﾀﾞ
//=============================================================================
int TPrinter::PrintQRCodeEx(char *txt,long lngErrorLevel,int pixPitch ,int x ,int y)
{
...
//	dblPitch = pitch * 56.7;
	...
	if( pixPitch <= 1 ) pixPitch=1;
//	printf("pitch2 = %f",pixPitch);
		...
		for( lngY = 0;lngY <= lngSell-1;lngY++){
			for( lngX = 0 ; lngX <= lngSell - 1; lngX++){
				if( lngResult[lngX * lngSell + lngY] >= 1 ){
					...
					SetRect( &rect,	sx + (int)(lngX * pixPitch),			sy + (int)(lngY * pixPitch),
									sx + (int)(lngX * pixPitch+pixPitch),	sy + (int)(lngY * pixPitch+pixPitch) );
