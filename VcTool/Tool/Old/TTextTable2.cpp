...
TTextTable2::TTextTable2(void)
{
	...
	m_EncryptionEnableFlg=l_EasyEnc->CheckEnc();
	delete l_EasyEnc;
...
BOOL TTextTable2::ReadFileEnc(  HANDLE hFile,  LPVOID lpBuffer,  DWORD nNumberOfBytesToRead, LPDWORD lpNumberOfBytesRead,   LPOVERLAPPED lpOverlapped )
{
	...
	if( ret != 0 && m_Encryption!=0 ){
		p=(char*)lpBuffer;
		for(i=0;i<(int)nNumberOfBytesToRead;i++){
			*p++ ^= (char)m_Encryption;
		}
	}
	return ret;
...
BOOL TTextTable2::WriteFileEnc( HANDLE hFile,  LPCVOID lpBuffer, DWORD nNumberOfBytesToWrite,LPDWORD lpNumberOfBytesWritten,LPOVERLAPPED lpOverlapped )
{
	...
	if( m_Encryption!=0 ){
		p=(char*)lpBuffer;
		for(i=0;i<(int)nNumberOfBytesToWrite;i++){
			*p++ ^= (char)m_Encryption;
		}
		if( m_EncryptionEnableFlg==0 ){
