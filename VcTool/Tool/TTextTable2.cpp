...
//	File Name TTextTable2.cpp
//	作成者：Coba
//	作成日：1999.05.10
//	修正日：2004.11.12 ﾏﾂﾀﾞ		ネットワークでの競合対策のため、Insert2,Del2,Upd2を追加
//	2006.10.11	Maeda	XOR 暗号化処理追加(暫定版)
//=======================================================================================================
#include "TTextTable2.h"


//---------------------------------------------------------------------------------------------------------------------
//	関数名：TTextTable2::TTextTable2(void)
//	メンバ：public
//	機能　：コンストラクタ
//---------------------------------------------------------------------------------------------------------------------
TTextTable2::TTextTable2(void)
{
	m_PrevRecNo = 0;
	m_EncryptionMode = 0;
	m_EncryptionEnableFlg=0;	//
	m_BufSize = DEF_BUFSIZE;


	// 暗号化処理が登録されているかチェックする
	CEasyEnc* l_EasyEnc;
	l_EasyEnc=NULL;
	l_EasyEnc=new CEasyEnc();
	m_EncryptionEnableFlg=l_EasyEnc->CheckEncKey();
	delete l_EasyEnc;
...
//---------------------------------------------------------------------------------------------------------------------
//	int TTextTable2::Initialize(FSPEC *filespec, int bufsize)
//	メンバ：protected
//	機能　：ファイル構造の設定
//	引数　：構造定義用の構造体”FSPEC”
...
int TTextTable2::Initialize(FSPEC *filespec, int bufsize)
{
	...
	// 簡易暗号化されているかチェック！！
	// 新規作成の場合は、暗号化可能
	// 既にあるファイルの時は、暗号化されているかチェックする
	m_EncryptionMode=0;

	if( m_EncryptionEnableFlg==0 ) return 0;	// レジストリにキーが無い時は無条件で駄目！！
	...
	if( fsize == 0 ){
		...
		ReadFileEnc( hFile, buff, 1, &dwRead, NULL );
		if(dwRead){
			if( CEasyEnc::CheckEncFile( buff ) == FALSE ){
				// ノーマルパターン
				m_EncryptionMode=0;
			}else{
				// 簡易暗号化されている
...
//---------------------------------------------------------------------------------------------------------------------
//	関数名：void TTextTable2::Clear(void)
//	メンバ：private
//	機能　：データバッファの初期化
//---------------------------------------------------------------------------------------------------------------------
void TTextTable2::Clear( void )
{
...
//---------------------------------------------------------------------------------------------------------------------
//	関数名：void TTextTable2::GetFirst( void *key )
//	メンバ：protected
//	機能　：最初のレコードを取得
//	引数　：検索キー
//	戻り値：0=成功、その他=エラー
//---------------------------------------------------------------------------------------------------------------------
int TTextTable2::GetFirst( void )
{
	HANDLE hFile;
	int ret;
	long nowrec=0;					// 現在のレコード位置
	...
	ReadFileEnc( hFile, m_fSpec.datptr, m_fSpec.reclen, &dwRead, NULL );
	if(!dwRead) ret = 41;
	m_PrevRecNo = 1;
	// Close
	Close(hFile);
...
//---------------------------------------------------------------------------------------------------------------------
//	関数名：void TTextTable2::GetEqual( void *key )
//	メンバ：protected
//	機能　：レコード検索/キーに合致するレコードを検索（ユニークキー対象）
//	引数　：検索キー
//	戻り値：0=成功、その他=エラー
//---------------------------------------------------------------------------------------------------------------------
int TTextTable2::GetEqual( void *key )
{
	HANDLE hFile;
	int ret;
	long nowrec=0;					// 現在のレコード位置
...
//---------------------------------------------------------------------------------------------------------------------
//	関数名：void TTextTable2::GetEqual2( void *key )
//	メンバ：protected
//	機能　：レコード検索/キーに合致するレコードを検索（ユニークキー対象）
//	引数　：検索キー
//	戻り値：0=成功、その他=エラー
//---------------------------------------------------------------------------------------------------------------------
int TTextTable2::GetEqual2( void *key )
{
	HANDLE hFile;
	int ret;
	long nowrec=0;					// 現在のレコード位置
...
//	戻り値：0=成功、その他=エラー
//---------------------------------------------------------------------------------------------------------------------
int TTextTable2::FindFirst( void *key )
{
	HANDLE hFile;
	int ret;
	long nowrec=0;					// 現在のレコード位置
	...
	if( !ret ){
		...
		while( nowrec > 1 ){
			...
			ReadFileEnc( hFile, prevKey, m_fSpec.keylen, &dwRead, NULL );
...
//---------------------------------------------------------------------------------------------------------------------
//	関数名：int TTextTable2::Upd( void *key )
//	メンバ：protected
//	機能　：
//	引数　：更新するキー
//	戻り値：0=成功、その他=エラー
//---------------------------------------------------------------------------------------------------------------------
int TTextTable2::Upd( void *key )
{
	HANDLE hFile;
	int ret;
	long nowrec=0;					// 現在のレコード位置
	...
	if(!ret){
		WriteFileEnc( hFile, m_fSpec.datptr, m_fSpec.reclen, &dwRead, NULL );
	}
...
//---------------------------------------------------------------------------------------------------------------------
//	関数名：int TTextTable2::Insert( void *key )
//	メンバ：protected
//	機能　：
//	引数　：削除するキー
//	戻り値：0=成功、その他=エラー
//---------------------------------------------------------------------------------------------------------------------
int TTextTable2::Insert( void *key )
{
	HANDLE hFile,hTmp;
	int ret;
	long nowrec=0;					// 現在のレコード位置
	...
	for( i=0; i < readCount; i++){
		...
		ReadFileEnc( hFile, readBuf, m_BufSize, &dwRead, NULL );
		WriteFileEnc( hTmp, readBuf, m_BufSize, &dwRead, NULL );
	}

	// 余り部分を読む
	memset( readBuf, 0, m_BufSize );
	ReadFileEnc( hFile, readBuf, (nowrec * m_fSpec.reclen ) % m_BufSize, &dwRead, NULL );
	WriteFileEnc( hTmp, readBuf, (nowrec * m_fSpec.reclen ) % m_BufSize, &dwRead, NULL );

	// 該当ﾚｺｰﾄﾞを書き込む
	WriteFileEnc( hTmp, m_fSpec.datptr, m_fSpec.reclen, &dwRead, NULL );
	...
	// 余り部分を読む
	memset( readBuf, 0, m_BufSize );
	ReadFileEnc( hFile, readBuf, (totalSize - (nowrec*m_fSpec.reclen )) % m_BufSize, &dwRead, NULL );
	WriteFileEnc( hTmp, readBuf, (totalSize - (nowrec*m_fSpec.reclen )) % m_BufSize, &dwRead, NULL );
...
//---------------------------------------------------------------------------------------------------------------------
//	関数名：int TTextTable2::Insert2( void *key )
//	メンバ：protected
//	機能　：Insertを改良し、処理前にマスタファイル名を変更し、処理中は他からのアクセスが出来ないよう変更
//	引数　：削除するキー
//	戻り値：0=成功、その他=エラー
//---------------------------------------------------------------------------------------------------------------------
int TTextTable2::Insert2( void *key )
{
	HANDLE hFile,hTmp;
	int ret;
	long nowrec=0;					// 現在のレコード位置
...
//---------------------------------------------------------------------------------------------------------------------
//	関数名：int TTextTable2::Del( void *key )
//	メンバ：protected
//	機能　：
//	引数　：削除するキー
//	戻り値：0=成功、その他=エラー
//---------------------------------------------------------------------------------------------------------------------
int TTextTable2::Del( void *key )
{
	HANDLE hFile,hTmp;
	int ret;
	long nowrec=0;					// 現在のレコード位置
	...
	if( !ret ){
		...
		// 余り部分を読む
		memset( readBuf, 0, m_BufSize );
		ReadFileEnc( hFile, readBuf, ( (nowrec-1) * m_fSpec.reclen ) % m_BufSize, &dwRead, NULL );
		WriteFileEnc( hTmp, readBuf, ( (nowrec-1) * m_fSpec.reclen ) % m_BufSize, &dwRead, NULL );
...
//---------------------------------------------------------------------------------------------------------------------
//	関数名：int TTextTable2::Del2( void *key )
//	メンバ：protected
//	機能　：Delを改良し、処理前にマスタファイル名を変更し、処理中は他からのアクセスが出来ないよう変更
//	引数　：削除するキー
//	戻り値：0=成功、その他=エラー
//---------------------------------------------------------------------------------------------------------------------
int TTextTable2::Del2( void *key )
{
	HANDLE hFile,hTmp;
	int ret;
	long nowrec=0;					// 現在のレコード位置
...
//---------------------------------------------------------------------------------------------------------------------
//	関数名：int TTextTable2::MoveRecord( HANDLE hFile, void *key )
//	メンバ：private
...
//						＊ファイルオープンされてることが前提！
//---------------------------------------------------------------------------------------------------------------------
int TTextTable2::MoveRecord2( HANDLE hFile,void *key ,long *recNo )
{
	int ret;
	...
	*recNo = 0;
	...
	while(1){
		...
		if( ReadFileEnc( hFile, keybuf, m_fSpec.keylen, &dwRead, NULL )==FALSE){
			ret = 2;
			break;
		}
...
//						＊ファイルオープンされてることが前提！
//---------------------------------------------------------------------------------------------------------------------
int TTextTable2::MoveRecord( HANDLE hFile,void *key ,long *recNo )
{
	int cmpstat,i;
	char keybuf[TXDB_MAXKEYLEN+1];
	long nowrec=0;					// 現在のレコード位置
	...
	*recNo = 0;
	...
	if( ReadFileEnc( hFile, keybuf, m_fSpec.keylen, &dwRead, NULL )==FALSE) return(2);
	...
	if(cmpstat){
		...
		for(i=0; i<2 ;i++ ){
			ReadFileEnc( hFile, keybuf, m_fSpec.keylen, &dwRead, NULL );
			cmpstat = memcmp( key, keybuf, m_fSpec.keylen );
...
//---------------------------------------------------------------------------------------------------------------------
//	関数名：void TTextTable2::Close(HANDLE& hFile)
//	メンバ：private
//	機能　：ファイルクローズ
//	引数　：Closeするファイルハンドル
//---------------------------------------------------------------------------------------------------------------------
void TTextTable2::Close(HANDLE& hFile)
{
	CloseHandle( hFile );
}

//---------------------------------------------------------------------------------------------------------------------
//	関数名：int TTextTable2::Open(HANDLE& hFile)
//	メンバ：private
//	機能　：ファイルオープン
//	引数　：オープンするファイルハンドル
//	戻り値：0=成功、その他=エラー
//---------------------------------------------------------------------------------------------------------------------
int TTextTable2::Open(HANDLE& hFile,BOOL readonly)
{
	DWORD dwCre;
	DWORD dwAcs;
	DWORD dwAttr;
...
//---------------------------------------------------------------------------------------------------------------------
int TTextTable2::GetCounts(void)
{
	if(m_fSpec.reclen<1) return(-1);
...
//---------------------------------------------------------------------------------------------------------------------
//	関数名：int TTextTable2::CheckFile(void)
//	メンバ：private
//	機能　：ネットワーク環境で使用時のための、マスタファイルとテンポラリファイルのチェック関数
//---------------------------------------------------------------------------------------------------------------------
int TTextTable2::CheckFile(void)
{
	char fname[MAX_PATH+1];
...
//---------------------------------------------------------------------------------------------------------------------
//	関数名：int TTextTable2::SetEncryption( int EncryptionCode )
//	メンバ：private
//	機能　：簡易暗号化のコード設定
//	2006.10.11
//---------------------------------------------------------------------------------------------------------------------
/*
int TTextTable2::SetEncryption( int EncryptionCode )
{
	m_EncryptionMode = EncryptionCode;

	return TRUE;
}
*/

BOOL TTextTable2::ReadFileEnc(  HANDLE hFile,  LPVOID lpBuffer,  DWORD nNumberOfBytesToRead, LPDWORD lpNumberOfBytesRead,   LPOVERLAPPED lpOverlapped )
{
	int ret;

	ret=ReadFile(  hFile,  lpBuffer,  nNumberOfBytesToRead, lpNumberOfBytesRead,   lpOverlapped );
	if( ret != 0 && m_EncryptionMode!=0 ){
		CEasyEnc::EncData((unsigned char*)lpBuffer,(int)nNumberOfBytesToRead);
	}
	return ret;
}


BOOL TTextTable2::WriteFileEnc( HANDLE hFile,  LPCVOID lpBuffer, DWORD nNumberOfBytesToWrite,LPDWORD lpNumberOfBytesWritten,LPOVERLAPPED lpOverlapped )
{
	int ret;

	if( m_EncryptionMode!=0 && m_EncryptionEnableFlg==0 ){
		MessageBox(NULL,"Program Key Err\nThis PC can't use for Mixing Infomarion System\nPls setup new key!!","Err",MB_OK);
		return -1;	// Err
	}

	if( m_EncryptionMode!=0 ){
		CEasyEnc::EncData((unsigned char*)lpBuffer,(int)nNumberOfBytesToWrite);
	}
