...
//WinPrintf("cc");
	i=(unsigned int)strlen(theDir);						// "*.EXE"の部分をカット
...
//	windowsドライブのNOKENVフォルダにあるiniﾌｧｲﾙから取得
//	2005.12.08 coba追加
//	引数１：取得バッファ
//	引数２：取得バッファ長(含むNull)
//	引数４：INIファイルの名前（拡張子つき。INIファイルは実行ディレクトリと同じ場所に作成して下さい)
...
int GetNokIniString( char *GetBuf,short BufSize, char *ProfileName, LPCTSTR SectionName ,LPCTSTR KeyName)
{
	...
	theDir[3]=0;
	sprintf( &theDir[3],"NOKENV\\%s",ProfileName );			// 引数３のプロファイル名と結合
...
//=======================================================================================================
//int WriteIniString( char *data,short datalen, char *ProfileName
//							, LPCTSTR SectionName ,LPCTSTR KeyName,BOOL execpathonly=FALSE)
...
//	引数４：INIファイル名のキー名
//			＊文字列なのですべて最後にNullを付加する事！！
//	引数５：ﾌｧｲﾙの有無にかかわらず、exeのパスに強制でファイル作成して書きこむ
//	返値　：成功＝０,失敗＝－１
//========================================================================================================
...
//=======================================================================================================
//int WriteNokIniString( char *data,short datalen, char *ProfileName, LPCTSTR SectionName ,LPCTSTR KeyName)
//	C:\NOKENVにある指定したiniファイルにデータを書き込む
//	引数１：取得バッファ
//	引数４：INIファイルの名前（拡張子つき。INIファイルは実行ディレクトリと同じ場所に作成して下さい)
//	引数５：INIファイル内のセクション名([]でくくられた名称)
//	引数４：INIファイル名のキー名
...
void WriteNokIniString( char *data, char *ProfileName, LPCTSTR SectionName ,LPCTSTR KeyName)
{
	...
	memset( theDir, 0, sizeof(theDir) );
	if( !GetWindowsDirectory( theDir,sizeof(theDir))) return;	// windowsﾃﾞｨﾚｸﾄﾘを取得

	// 2006.12.12 ﾃﾞｨﾚｸﾄﾘを検索してないときは作成
	WIN32_FIND_DATA fd;
	HANDLE hFind;
	theDir[3]=0;
...
//	引数で指定されたフォルダを作成する。相対パス、絶対パスどちらでも可能
//	引数１：作成するフォルダ
//	戻り値：0=作成成功
//---------------------------------------------------------------------------------------------------------------------
int MakeFolders( char* path )
{
	...
	while(1){
		...
		// 失敗した場合は一階層上へ
			...
			// フォルダの最後の'\'までをチェックする
			for(i=(unsigned int)strlen(tmpDir)-1;i>=0;i--){
				if( tmpDir[i]=='\\' ){
					tmpDir[i]=0;
					break;
				}
