...
//	引数３：グリッド設定情報構造体
//---------------------------------------------------------------------------------------------------------------------
BOOL CGridViewCtrl::CreateGrid(LPRECT pRect,GRIDINFO* gr)
{
	// グリッド設定
	memset( &m_info, 0, sizeof(m_info) );
...
*/
	...
	if(movenext && action == 2 ){
		// まずはデータをセット
		if( !SetCel( m_editX,m_editY,theData,TGridData::GetCelTag(m_editX,m_editY), TRUE ,color,0)) return(FALSE);
		// 移動先が有効な場合はそこに飛ぶ
		if( nextX < TGridData::m_TotalCols && nextY <= TGridData::m_LastRowNo){
			m_ActCel.col=nextX;m_ActCel.row=nextY;
			// 次の編集先が指定されていたらそこに飛ぶ
			MoveActiveCel(nextX,nextY);
...
//---------------------------------------------------------------------------------------------------------------------
//	BOOL CGridViewCtrl::SetCel(int col,int row, char* data, long cTag,BOOL draw)
//	タイプ：public
//	機能　：セルにデータを設定
//	引数１：設定する列番号
//	引数２：設定する行番号
...
void CGridViewCtrl::EvVScroll(HWND hWnd,WORD event,WORD lastPos)
{
	...
	// IBMのマウスで飛ぶのでここで処理する 2007.11.28
...
BOOL CGridViewCtrl::MoveCel(int mvMode)
{
	...
	return(MoveActiveCel(nextCol,nextRow));
}
...
BOOL CGridViewCtrl::ColsCopy(long targetCol)
{
	...
	// 列情報をコピー
	memcpy( &m_ColInfo[COL_LIMIT], &m_ColInfo[targetCol], sizeof(COLUMNPOSITIONINFO) );
	return(TRUE);
}
...
BOOL CGridViewCtrl::ColsPaste(long targetCol,BOOL insert,BOOL redraw)
{
	...
	if( targetCol > prevCols || !insert){
		...
		// コピーする
		memcpy( &m_ColInfo[targetCol], &m_ColInfo[COL_LIMIT], sizeof(COLUMNPOSITIONINFO) );
	}

	// 挿入
	else{
		// ずらす場合（列MAX）
		if( prevCols == COL_LIMIT ) offset =1;
...
BOOL CGridViewCtrl::ColsCut(long targetCol ,BOOL redraw)
{
	...
	memset( &m_ColInfo[ prevCols-1], 0 ,sizeof(COLUMNPOSITIONINFO) );
