...
//------------------------------------------------------------------------------------------
// 背景色の変換テーブル
static const COLORREF gr_bk_clr[]=
{
	CLR_WHITE,
	CLR_DGRAY,
	CLR_BLUE,
...
BOOL CGridViewCtrl::CreateGrid(LPRECT pRect,GRIDINFO* gr,char* fontname,DWORD charset)
{
	...
	// 2014.10.04 フォントbold指定追加
	int fw;
	if( m_info.fontBold ) fw=FW_BOLD;
	else fw=FW_NORMAL;
...
///
	...
	// 通常のセル
	else if(!TGridData::m_StrData[row].bkcolor[col]){
		FillRect( m_hdcList,&rect,(HBRUSH)GetStockObject(WHITE_BRUSH));
	}
	// 2015.01.16 coba 色付け
	else if( GVW_BKC_GRAY < TGridData::m_StrData[row].bkcolor[col] && TGridData::m_StrData[row].bkcolor[col] <GVW_BKC_MAX){
		HBRUSH hbrush=CreateSolidBrush(gr_bk_clr[TGridData::m_StrData[row].bkcolor[col]]);
		FillRect( m_hdcList,&rect,hbrush);
		DeleteObject(hbrush);
	}
	// 濃い灰色
...
//			uFlags = DT_CENTER|DT_SINGLELINE;
	...
	if( act ){
		...
		// 色指定コバ変更 2006.12.14
		if(TGridData::m_StrData[row].color[col]){
			fcolor=this->GetColorFromId(TGridData::m_StrData[row].color[col]);
			old = SetTextColor( m_hdcList, fcolor );
		}
		DrawText(m_hdcList,txtBuf,-1,&rect,uFlags);
		if(TGridData::m_StrData[row].color[col]){
			SetTextColor( m_hdcList, old );
		}
/* 2006.12.14 コメントアウト by こば
	// 色つき
		if(TGridData::m_StrData[row].color[col]==1){
			old = SetTextColor( m_hdcList, CLR_RED );
		}
		// 2006.08.03 追加 ﾏﾂﾀﾞ
		else if(TGridData::m_StrData[row].color[col]==2){
			old = SetTextColor( m_hdcList, CLR_BLUE );
		}
	...
	//	if(TGridData::m_StrData[row].color[col]==1){
		// 2006.08.03 追加 ﾏﾂﾀﾞ
		if(TGridData::m_StrData[row].color[col]==1 || 
			TGridData::m_StrData[row].color[col]==2){
			SetTextColor( m_hdcList, old );
		}
*/
	}
...
//			uFlags = DT_CENTER|DT_SINGLELINE;
	...
	// 色戻し
	if(TGridData::m_StrData[row].color[col]){
		old = SetTextColor( m_hdcList, old );
		//old = SetTextColor( m_hdcList, fcolor ); ????
	}
/* 2006.12.14 コメントアウト by こば
	// 色つき
...
//				else if(!TGridData::m_StrData[datrow].bkcolor[col]) FillRect( m_hdcList,&rect,GetStockObject(WHITE_BRUSH));
				...
				// 2015.01.16 coba 色付け
				else if( GVW_BKC_GRAY < TGridData::m_StrData[datrow].bkcolor[col] && TGridData::m_StrData[datrow].bkcolor[col] <GVW_BKC_MAX){
					HBRUSH hbrush=CreateSolidBrush(gr_bk_clr[TGridData::m_StrData[datrow].bkcolor[col]]);
					FillRect( m_hdcList,&rect,hbrush);
					DeleteObject(hbrush);
				}
				else FillRect( m_hdcList,&rect,(HBRUSH)GetStockObject(GRAY_BRUSH));
...
//						uFlags = DT_CENTER|DT_SINGLELINE|DT_WORDBREAK;
				...
				// 通常のセル
					...
					// 色指定コバ変更 2006.12.14
					if(TGridData::m_StrData[datrow].color[col]){
						fcolor=this->GetColorFromId(TGridData::m_StrData[datrow].color[col]);
						old = SetTextColor( m_hdcList, fcolor );
					}
					DrawText(m_hdcList,txtBuf,-1,&rect,uFlags);
					if(TGridData::m_StrData[datrow].color[col]){
		...
		/* 2006.12.14 コメントアウト by こば
					...
					// 2006.08.03 追加 ﾏﾂﾀﾞ
					else if(TGridData::m_StrData[datrow].color[col]==2){
						old = SetTextColor( m_hdcList, CLR_BLUE );
					}
	...
	//				if(TGridData::m_StrData[datrow].color[col]==1){
					if(TGridData::m_StrData[datrow].color[col]==1 || 
						TGridData::m_StrData[datrow].color[col]==2){
						SetTextColor( m_hdcList, old );
					}
*/
				}
...
void CGridViewCtrl::EvVScroll(HWND hWnd,WORD event,WORD lastPos)
{
	...
	// 2007.02.22 IBMのホイールマウスで不正な処理対策のため、ここに移動
...
//		case WM_IME_KEYDOWN:
		...
		case WM_MOUSEWHEEL:		// 2013.06.19 coba
...
//	機能　：定数からRGBを引く
//---------------------------------------------------------------------------------------------------------------------
COLORREF CGridViewCtrl::GetColorFromId(int id)
{
	switch( id ){
		case GVW_CLR_BLACK:
			return(CLR_BLACK);
