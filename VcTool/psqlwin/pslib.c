...
// クライアントエンコード保存バッファ 2010.05.20 coba
static char g_encoding[100];
...
{
	// クライアントエンコードのデフォ設定 2010.05.20 coba
	strcpy(g_encoding,"SHIFT_JIS");
...
int PS_ReConnect(PGconn** pg)
{
	...
	// エンコーディング再設定 2010.05.20 coba
	if(memcmp(g_encoding,"SHIFT_JIS",strlen(g_encoding))){
		PS_Encoding(*pg,g_encoding);
	}
...
int PS_GetColName( PGresult* res, int colno,char* fieldname,int bufsize)
{
	...
	len=(unsigned int)strlen(PQfname( res, colno));
	if( len <1 ) return(-2);
...
//--------------------------------------------------------------------------------------------------------------------
// char *PS_ColbyNo( PGresult* res, int rowno,int colno)
// 説明　:行とフィールド名を指定して、フィールドデータの取得(select * 検索向け)
// 引数１:　結果セット
// 引数２:　取得したい行の番号
// 引数３:　取得したいフィールド名
//	2011.02.01 matsuda どうにもPS_Col内のPQfnumberでは
//	 フィールド番号を見つけてくれない環境依存文字「№」に対応するため
//	 フィールド番号を直接指定するため用意
//--------------------------------------------------------------------------------------------------------------------
char *PS_ColbyNo( PGresult* res, int rowno,int colno)
{
...
// 説明　:クライアント側エンコーディングを設定
// 引数１:データベース接続変数
// 引数３:設定するエンコード ex "SQL_ASCII","SHIFT_JIS","GBK"(中文)
// 戻り値: 0=成功  それ以外＝失敗
//--------------------------------------------------------------------------------------------------------------------
int PS_Encoding( PGconn* pg, char* encoding)
{
	...
	sprintf( sql,"set client_encoding to '%s'",encoding);
	
	// 下の関数はlipqの更新が必要なので却下
	//ret = PQsetClientEncoding(pg, encoding);/* エンコード設定 */
	res = PQexec(pg, sql);/* 問い合わせの送信 */
	if( res == NULL )return(1);
	if (PQresultStatus(res) != PGRES_COMMAND_OK){
		PQclear(res);
