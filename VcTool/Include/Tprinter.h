...
//		修正日：2007.02.22 coba
//					文字を重ねがきしたときに背後の文字がつぶれないようにSetBkMode()をする
//		修正日：2007.05.07 Maeda
//					ConvertMmToPixcel() をPublicに変更
//		修正日：2008.09.03 coba CODE39のテーブルの間違えを修正
//		修正日：2009.07.06 ryuu PrMsgEx関数の前面色/背景色出力関数を新作（中国語フォントカラー出力用）
//		修正日：2010.11.23 matsuda 特別管理記号の追加のためPrGaijiに変わるPrTokubetuを追加(非Btimap)
//		修正日：2012.03.17 ryuuWNF用特別記号追加(PrTokubetu) tokubetu_no 4,5,6
//		修正日：2012.05.25 matsuda DLLを使わないQR印字関数を追加
//		修正日：2012.06.19 coba 初期化関数(InitPageSettingEx())を2個追加
//								プリンタ指定で用紙はデフォルト、用紙サイズ&向き指定
//		修正日：2012.06.25 coba 画像ファイル(bmp,jpg)印刷関数追加 DrawImgFile()
//		修正日：2012.08.24 coba PrBcode128()ピクセル指定版追加
//              修正日：2013.06.06 kobashi DrawCircle()に色指版定追加
//		修正日：2013.07.23 sanno 印刷時のファイル名を指定できるコンストラクタ追加
//		修正日：2013.08.19 matsuda CenterTextのバグを修正し、関数名をCenterMsgに変更
//		修正日：2014.07.18 sanno CenterMsgで文字の前面色/背景色を指定できるよう変更
//		修正日：2015.07.02 tani  DrawRに色指定追加
//===============================================================================================
...
class TPrinter
{
	public :					// どの関数からもアクセス可能==================
		...
		//	コンストラクター(保存ファイル名指定)	//2013.07.23 sanno
		TPrinter(LPSTR theDrv,LPSTR theDev,LPSTR thePort ,int *initStat,short pageSize,short orient, const char* DocName);
...
// 2013.08.19 matsuda この関数はバグがあるため、使用不可。同じ引数を使う場合には、CenterMsgを使用すること
...
//						LPSTR Message );	// 文字印刷列
		...
		void CenterMsg(int	x,				// Ｘ座標(0.1mm)
						int	y,				// Ｙ座標(0.1mm)
						int	endX,			// 終了座標(0.1mm)
						int	nFont,			// フォントの番号(登録済み)
						int nBold,			// フォント太さ(VC定数)
		...
		//前景／背景色を指定したCenterMsg関数追加	//2014.07.18 sanno
		void CenterMsg(int	x,				// Ｘ座標(0.1mm)
						int	y,				// Ｙ座標(0.1mm)
						int	endX,			// 終了座標(0.1mm)
						int	nFont,			// フォントの番号(登録済み)
		...
		// 2012.08.24 coba ピクセル指定版
		void PrBCode128Pix( int x, int y, int nWidth, int nHeight,int strON ,LPSTR Message ,int code);
	...
	// 派生クラスとフレンドのみアクセス可能========
		...
		void DrawCircle( int x, int y ,int rx ,int ry , int LineTye ,int width,COLORREF color);
		...
		void DrawR( int x, int y ,int r ,int mode , int LineType ,int width,COLORREF color=0);

		//BitMapを描画する 2004.12.08 ﾏﾂﾀﾞ追加
		void DrawBitmap( int x, int y ,int nWidth,int nHeight, LPBITMAPINFO bmpinfo );

		// ファイルから画像を読み出して印字 2012.06.25 coba
		int DrawImgFile(const char* fname,const int x,const int y,const int nWidth,const int nHeight);
	...
	protected :
		...
		// 2012.06.19 coba プリンタ指定、用紙・向きetcはプリンタ設定に従う
		int	InitPageSettingEx(LPSTR theDrv,LPSTR theDev,LPSTR thePort);

		// 2012.06.19 coba プリンタ・用紙サイズ・向き指定
		int InitPageSettingEx2(LPSTR theDrv,LPSTR theDev,LPSTR thePort ,short theLength, short theWidth,short orient);
		...
		int	InitPageSettingEx(LPSTR theDrv,LPSTR theDev,LPSTR thePort ,short thePage, short theCopies, short tray);
			...
			void PrTokubetu( int x, int y, int tokubetu_no );	//2010.11.23 matsuda
...
//==================================================================================================
	public:
		int PrintQRCodeEx(	char *txt,			//char *txt	印字文字(最大文字数256)
							long lngErrorLevel,	//long lngErrorLevel	誤り訂正レベル L(7%),M(15%),Q(25%),H(30%)
							int pitch,	//1セルあたりの大きさを直接指定
							int	 x,				// Ｘ座標(0.1mm)
							int  y);				// Ｙ座標(0.1mm)
		...
		//2007.5.7 private-->public
		int	ConvertMmToPixcel( int mm , int xy );

//==================================================================================================
//==================================================================================================
//		以下、本体は TPrintQREx.cppにあるので、使う場合はTPrinQREx.cppを追加し、
//		さらに同フォルダのQR_Encode.cppも追加すること
//==================================================================================================
//==================================================================================================
		int PrintQR(	char *txt,			//char *txt	印字文字(最大文字数256)
							long lngErrorLevel,	//long lngErrorLevel	誤り訂正レベル L(7%),M(15%),Q(25%),H(30%)
							int pitch,	//1セルあたりの大きさを直接指定
							int	 x,				// Ｘ座標(0.1mm)
							int  y);				// Ｙ座標(0.1mm)
