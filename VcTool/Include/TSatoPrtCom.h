#ifndef SATO_LABEL_COM_PRINTER
#define SATO_LABEL_COM_PRINTER
//=====================================================================================================================
//
//  	さとーバーコードプリンターの制御基本クラス(シリアルインターフェース版)
//	
...
{
	public:
		// デフォのコンストラクタ。この場合はInitPrinter()を呼ぶ
		TSatoPrinter();
		BOOL InitPrinter(int ptrtype,int portNo, int speed,int data,int stop,int parity);
		// 単体のコンストラクタ
		TSatoPrinter( int ptrtype,int portNo, int speed,int data,int stop,int parity, BOOL* status );
