...
//		for( i= 0; i <= m_LastRowNo ; i++ ){
			...
			m_StrData[i].celTag=newtag;
			newpt = (char*)realloc(m_StrData[i].color, m_TotalCols+1);
			m_StrData[i].color=newpt;
			newpt = (char*)realloc(m_StrData[i].bkcolor, m_TotalCols+1);
			m_StrData[i].bkcolor=newpt;
...
//			memcpy(m_StrData[i].d[j],m_StrData[i+1].d[j],1);
			...
			m_StrData[i].color[j]	= m_StrData[i+1].color[j];
			m_StrData[i].bkcolor[j]= m_StrData[i+1].bkcolor[j];
	...
	for(j=0;j<m_TotalCols+1;j++){
		...
		m_StrData[m_LastRowNo].bkcolor[j]=0;
		m_StrData[m_LastRowNo].color[j]=0;
...
long TGridData::InsertNewRow(long row,long tag,char* title)
{
	...
	for(j=0; j < m_TotalCols+1;j++){
		...
		m_StrData[row].bkcolor[j]=0;
		m_StrData[row].color[j]=0;
